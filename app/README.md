...
The **Schema** tab is a routine manager: a library of **named routines** (Push, Pull, Legs, Upper A…) independent of the weekday, each with editable title/label and a reorderable exercise list. The **Training** tab lets you pick which routine to do today and start it; there's no fixed weekday coupling, so PPL rotations, A/B splits or "whatever's next" all work. A workout stays separate from the routine template:
...
Not doing a listed routine still offers a quick "markeer als afgerond" stamp. Existing weekday schemas are migrated to named routines automatically on first load (after sync settles, so nothing is overwritten). This split (routine library ↔ dated session log) is what enables real history and progression, rather than checkboxes on a recurring schedule.
...
Each logged item stores its macros *per 100 g/ml*, so editing the amount rescales the calories and macros automatically. Where Open Food Facts provides them, the app also captures **fibre, sugar, saturated fat and salt**, shown as a daily "Micronutriënten" strip. A per-day **water tracker** (quick +250/+500 ml, 2 L goal) sits under the day summary. A large built-in list of common whole foods covers what Open Food Facts handles poorly. Foods you've logged before appear under "Recent"; you can also **save a meal** (a combination of foods) and re-add it in one tap. Bodyweight is shown with a smoothed **trend line** (EWMA) alongside the raw measurements, since the trend is what matters for decisions. Nutrition data — including recents and saved meals — is saved in the same store as the training schema and rides along on the optional Supabase sync.
